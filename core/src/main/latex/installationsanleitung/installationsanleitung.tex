...
\newcommand{\vwiaversfassungnr}{7.9}
\newcommand{\vwiaversfassungmonat}{September}
\newcommand{\vwiaversfassungjahr}{2020}

% VerA.web Version
\newcommand{\vwiaverssw}{2.4}
...
VerA.web-Distribution“) beziehen:\keinumbruch
...
Diese Dateien werden bei der Kompilierung von VerA.web erzeugt.
...
Auf Fedora Linux 34 und höher ist die Deaktivierung nicht mehr möglich.
Fügen Sie \texttt{selinux=0} zur Kernelkommandozeile hinzu:
\texttt{sudo grubby --update-kernel=ALL --args=selinux=0}
...
cd veraweb-core-〈\lstdumpesc{\vwiaverssw}〉-files
chmod go-rwx .
\end{lstdump}
\end{minipage}
...
\begin{lstdump}{Adresse einzelner Mandanten setzen}
...
Der folgende Abschnitt ist abhängig von Ihrer Tomcat-Version. Falls Sie
mindestens\label{manual:tcajpcve} Tomcat 7.0.100, 8.0.14-1+deb8u17, 8.5.51,
9.0.31 oder später einsetzen müssen Sie im folgenden \emph{beide} Attribute
hinzufügen, da sonst der Applikationsserver mit einer Fehlermeldung ähnlich
„\texttt{java.lang.IllegalArgumentException: The AJP Connector is configured
with secretRequired="true" but the secret attribute is either null or "".
This combination is not valid.}“ nicht vollständig starten wird. Bei einer
älteren Version als den obengenannten existiert das neue Attribut
\texttt{secretRequired} noch nicht; lassen Sie es dort einfach weg.
...
Entfernen Sie hier die Kommentarzeichen und fügen Sie zwei weiteres Attribute
...
<Connector address="127.0.0.1" secretRequired="false" port="8009" protocol="AJP/1.3" redirectPort="8443" />
\end{lstdump}
\end{minipage}
...
cd ..
rm -rf veraweb-core-〈\lstdumpesc{\vwiaverssw}〉-files* veraweb.war vwor.war
\end{lstdump}
\end{minipage}
...
\subsection{Upgrade von Versionen vor 2.3}\label{subsec:upgrade-core-before-tcajpcve}

Es sind mittlerweile Sicherheitsupdates für den Applikationsserver
Tomcat erschienen, die beim Start eine Fehlermeldung loggen und nicht
vollständig starten. Bitte sehen Sie in Kapitel \ref{manual:tcajpcve},
Listing 22, wie Sie Ihre \texttt{server.xml} anpassen, um auch mit
einer aktuellen Tomcat-Version den Betrieb wiederherzustellen.
