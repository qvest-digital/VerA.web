...
\begin{tikzpicture}[
  ...
  microsvc/.style={rectangle,draw=Orange,minimum width=32mm},
  webapp/.style={rectangle,draw=red,minimum width=32mm},
  syssvc/.style={rectangle,draw=Violet,minimum width=25mm},
  script/.style={rectangle,draw=black,minimum width=25mm},
  people/.style={rectangle,draw=CornflowerBlue,minimum width=12mm},
  ...
  \node[script]   (usql)   at (  5mm,38mm) {upgrade.sql};
  \node[script]   (riosh)  at (130mm,19mm) {register-into-osiam.sh};
  \node[people]   (admins) at (130mm,99mm) {Admins};
  \node[people]   (sb)     at (130mm,82mm) {Sachbearbeiter};
  \node[people]   (guests) at (155mm,46mm) {Gäste};
  \node[acloud]   (inet)   at (130mm,48mm) {};

  \draw[thick,->] (admins) -- ([yshift=3mm]alam);
  \draw[thick,->] (alam) -- (ldap);
  \draw[thick,->] ([yshift=-3mm]admins) -| +(-15mm,0mm) |- ([yshift=3mm]acore);
