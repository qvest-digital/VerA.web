...
\begin{tikzpicture}[
  ...
  \draw[dashed]   (0mm,32mm) -- (175mm,32mm);
...
\begin{tikzpicture}[
  ...
  \fill[yellow!40] (3mm,33mm) -- (122mm,33mm) -- (122mm,48mm) -- (3mm,48mm);
  \fill[green!20] (3mm,50mm) -- (122mm,50mm) -- (122mm,94mm) -- (3mm,94mm);

  \node[above right,rectangle,fill=blue!20,minimum width=119mm,minimum height=15mm] (ldap) at (3mm,96mm) {LDAP};
  \node[above right,rectangle,fill=red!20,minimum width=169mm,minimum height=15mm] (osiam) at (3mm,10mm) {OSIAM};

  \node[microsvc] (svwoa)  at ( 40mm,36mm) {VerA.web Online-Anmeldung};
  \node[webapp]   (score)  at ( 40mm,82mm) {VerA.web Webanwendung};
  \node[webapp]   (svwor)  at ( 40mm,64mm) {VerA.web REST-API};
  \node[syssvc]   (dbvw)   at (  5mm,53mm) {veraweb DB};
  \node[people]   (admins) at (168mm,99mm) {Admins};
  \node[people]   (sb)     at (168mm,82mm) {Sachbearbeiter};
  \node[people]   (guests) at (168mm,36mm) {Gäste};
  \node[acloud]   (inet)   at (130mm,38mm) {};
