...
\begin{tabular}{| r || l | l | l |}\hline
 ...
 Applikationsserver    & Tomcat 7.0.28-4+deb7u9 & aktuelle 7.x & Tomcat 8\Hair\textsuperscript{\ref{fn:req-notmore}}\\\hline
...
„trusty“) oder einer anderen Java/Tomcat-Umgebung (z.B. Solaris \dash
...
wir die tar.gz-Variante\Hair\textsuperscript{\ref{fn:tarxzf}}) sowie die
...
\fi% !oa
(oder deren ZIP-Äquivalent; in dieser Anleitung verwenden
wir die tar.gz-Variante\Hair\footnote{\label{fn:tarxzf}auf Solaris-Systemen
stattdessen \texttt{zcat veraweb-core-\vwiaverssw{}-files.tgz | tar -xf -}
verwenden}) sowie die
Installationsarchive \texttt{veraweb-core-\vwiaverssw{}.war}
und \texttt{rest-api-\vwiaverssw{}.war} auf den core-Server.
...
\begin{lstdump}{/etc/veraweb/*.properties}
sudo install -c -o 0 -g tomcat7 -m 640 vwor.properties /etc/veraweb/
\end{lstdump}
\end{minipage}
...
Mit dem Eintrag imprint.<language\_key>.<sorting\_number><space><titel>=<text>
...
Aus den drei Teilen <sorting\_number>, <space> und <titel> setzt sich die Überschrift
des Absatzes im fertigen Impressum zusammen. Durch das Nummerieren der Abschnitte kann
die vorgesehene Reihenfolge erzeugt werden. Um ein Leerzeichen zwischen der Nummerierung
und dem Titel zu erzeugen muss die Unicode-Form \textbackslash{}u00A0 verwendet werden.
Andere Umlaute wie ä, ö, ü, Ä, Ö, Ü, ß,~… müssen auch dementsprechend kodiert werden.

Der eigentliche Text des Abschnitts (<text>) kann mit Leerzeichen umgehen. Umlaute
müssen aber weiterhin codiert werden.

Um nur einen Textblock ohne Überschrift zu generieren können <sorting\_number>,
<space> und <titel> weggelassen werden. Der Eintrag muss dann wie folgt
gestaltet werden: \texttt{imprint.de\_DE.=<text>} bzw. \texttt{imprint.en\_EN.=<text>}
(die Punkte können nicht weggelassen werden).

Sollte es mehrere Einträge ohne Sortiernummer oder mit doppelter Nummer geben,
ist keine spezielle Reihenfolge gewährleistet.

\begin{lstdump}{/etc/veraweb/vwor.properties}
...
imprint.de_DE.1\u00A0\u00DCberschrift=Text des ersten Abschnitts
imprint.de_DE.3\u00A0Absatz=Text des dritten Abschnitts
imprint.de_DE.2\u00A0Titel=Diese Seite geh\u00F6rt dem Spa\u00DF-Verein

imprint.en_EN.=Just text without headline.
\end{lstdump}
