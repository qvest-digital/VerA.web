...
public class EmailResource extends AbstractResource {
    ...
    public void sendEmailVerification(@FormParam("email") String email, @FormParam("endpoint") String endpoint, @FormParam("activation_token") String activation_token, @FormParam("language") String currentLanguageKey) throws MessagingException {
        ...
        mailDispatcher.send(emailConfiguration.getFrom(), email, emailConfiguration.getSubject(), emailConfiguration.getContent(), getActivationLink(endpoint, activation_token), emailConfiguration.getContentType());
    }
